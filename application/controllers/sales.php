...
class Sales extends Secure_area
{
		...
		if (!empty($itens)) {
			foreach ($itens as $key => $iten) {
				...
				$verifydesconto = $this->db->get();
				 ...
				 $customerDetail = $this->Customer->get_info($customer_id);
				if ($verifydesconto->num_rows() == 0) {


					$iten['discount'] = $customerDetail->desconto;
					$itens[$key] = $iten;
				}

			
				if($this->validarCNPJ($customerDetail->document) && $iten['valor_cnpj'] != ""){
					$itens[$key]['price'] = $iten['valor_cnpj'];
	...
	function add()
	{
		...
		if ($this->sale_lib->check_replacement_level($item_id_or_number_or_item_kit_or_receipt)) {
			$data['warning'] = $this->lang->line('sales_check_replacement_level');
		}
		$this->_reload($data);
	}
