...
class Detailed_sales extends Report
{
	...
	public function getData(array $inputs)
	{
		...
		foreach ($data['summary'] as $key => $value) {
			$this->db->select('name, categories_products.category_name as category, quantity_purchased, serialnumber, sales_items_temp.description, subtotal, total, tax, profit, discount_percent');
			$this->db->from('sales_items_temp');
			$this->db->join('items', 'sales_items_temp.item_id = items.item_id');
			$this->db->join('categories_products', 'items.category_id = categories_products.category_id', 'left'); // JOIN para obter o nome da categoria
